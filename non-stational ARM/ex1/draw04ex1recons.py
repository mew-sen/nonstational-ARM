...
# 定义一个示例函数 u_finite
# u_finite = Expression('(1-pow(x[0],2)-pow(x[1],2))*exp(x[0]+x[1])', degree=4)
u_finite = Expression(
    'pow((1 - pow(x[0],2) - pow(x[1],2)),2) * std::log(x[0] + 3)',
    degree=4
)
...
# 定义表达式
# u_finite = Expression(
#     "normal_const * cos(2 * atan2(x[1], x[0])) * jn(2,j21*sqrt(x[0]*x[0]+x[1]*x[1]))",
#     degree=4,
#     normal_const=normal_const,j21=j21
# )
...
data_plot = np.column_stack((X_vector,Y_vector,Z_vector))
np.savetxt('data_plot_recons.txt', data_plot)
###########################################################################
##################           绘图并且比较两个图             #################
###########################################################################
